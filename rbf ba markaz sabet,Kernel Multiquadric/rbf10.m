...
%sabet va kernel Multiquadric baraye darunyabi estefade gardid.in marakez be surate random ijad
%mishavand.
close all
clear all
clc;
...
p=sort(v);
markaz=p';
% sakhte matrix darunyabi baraye data training va taabe kernel e
% Multiquadric
p=length(x);
p1=length(markaz);
for i=1:1:p
...
% sakhte matrix darunyabi baraye data test va taabe kernel e Multiquadric
p2=length(xt);
for i=1:1:p2
